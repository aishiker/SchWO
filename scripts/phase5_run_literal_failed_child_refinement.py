...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the bounded point-only literal failed-child refinement"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("runs/phase5/fig5_fig6_literal_failed_child_refinement"),
    )
    parser.add_argument(
        "--accepted-review-npz",
        type=Path,
    ...
    parser.add_argument(
        "--radial-gate-dir",
        type=Path,
        default=Path("runs/phase5/fig5_fig6_literal_failed_child_radial_gate"),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    except LiteralFailedChildRefinementContractError as exc:
        print(f"literal failed-child contract error: {exc}")
        return 2
    return 0
