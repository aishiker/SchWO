#!/usr/bin/env python3
"""Check, publish, or reload the bounded Phase-6 V2.2 v2 evidence root."""
...
from schwgw.validation.phase6_v2_waveform_routes import (  # noqa: E402
...
def _parser() -> argparse.ArgumentParser:
    ...
    commands.add_parser("check", help="verify inputs and build all records in memory")
    publish = commands.add_parser("publish", help="publish once to a fresh root")
    publish.add_argument("--output-root", type=Path, required=True)
    publish.add_argument("--verification-json", required=True)
    reload_parser = commands.add_parser("reload", help="strictly reload a sealed root")
...
def main() -> int:
    ...
    if args.command == "check":
        ...
        result = {
            "frozen_input_count": len(gate["frozen_hashes"]),
        ...
        result = publish_v2_2_waveform_routes(
            project_root=ROOT,
            output_root=args.output_root,
            verification=verification,
        )
    else:
        reloaded = validate_published_v2_2_waveform_routes(args.root, project_root=ROOT)
        result = {
